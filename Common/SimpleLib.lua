...
function CheckUpdate()
    if AUTOUPDATES then
        ...
        ToUpdate.VersionPath = "raw.githubusercontent.com/LegendBot/iCreative/master/version/SimpleLib.version"
        ToUpdate.ScriptPath = "raw.githubusercontent.com/LegendBot/iCreative/master/SimpleLib.lua"
...
function _Spell:IsReady()
    ...
    return self.Slot ~= nil and myHero:CanUseSpell(self.Slot) == READY
end
...
function _Spell:LastHit(tab)
    ...
    if self.EnemyMinions ~= nil and (self:IsSkillShot() or self:IsTargetted() or self:IsSelf()) and mode ~= LASTHIT_MODE.NEVER and self:IsReady() then
        if self.SourceFunction ~= nil then
            self.EnemyMinions.fromPos = self.Source
        end
        if self.RangeFunction ~= nil then
...
function _Prediction:GetPrediction(target, sp)
    ...
    if target ~= nil and IsValidTarget(target, math.huge, target.team ~= myHero.team) and self:ValidRequest(TypeOfPrediction) then
        ...
        -- VPrediction
        if TypeOfPrediction == "VPrediction" or not target.type:lower():find("hero") then
...
function _OrbwalkManager:__init()
    ...
    AddCreateObjCallback(
        function(obj)
            if myHero.range > 300 and self.AA.Object == nil and obj.name:lower() == "missile" and self:GetTime() - self.AA.LastTime + self:Latency() < 1.2 * self:WindUpTime() and GetDistanceSqr(Vector(myHero.x, 0, myHero.z), Vector(obj.x, 0, obj.z)) < 80 * 80 then
                --self:ResetMove()
                self.AA.Object = obj
    ...
    AddDeleteObjCallback(
        function(obj)
            if myHero.range > 300 and obj and self.AA.Object ~= nil and obj.name == self.AA.Object.name and obj == self.AA.Object then
                self:ResetMove()
            end
        end
    )
...
function _OrbwalkManager:TakeControl()
    ...
    --if VIP_USER then _G.OrbwalkManagerMenu:addParam("BlockMove","Block Move When Attacking", SCRIPT_PARAM_ONOFF, false) end
    ...
    if VIP_USER then 
        AddSendPacketCallback(
            function(p)
                p.pos = 2
                if _G.OrbwalkManagerMenu ~= nil and _G.OrbwalkManagerMenu.BlockMove and myHero.networkID == p:DecodeF() then
                    if self:IsAttacking() and not IsEvading() then
                        Packet(p):block()
                    end
                end
            end
